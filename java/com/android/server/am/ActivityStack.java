...
final class ActivityStack {
    ...
    /**
     * The back history of all previous (and possibly still running) activities.
    ...
    /**
     * List of activities that are waiting for a new activity to become visible
    ...
    /**
     * List of activities that are ready to be stopped, but waiting for the next
     ...
     */
    final ArrayList<IActivityManager.WaitResult> mWaitingActivityLaunched = new ArrayList<IActivityManager.WaitResult>();

    /**
     * List of people waiting to find out about the next visible activity.
     */
    final ArrayList<IActivityManager.WaitResult> mWaitingActivityVisible = new ArrayList<IActivityManager.WaitResult>();

    final ArrayList<UserStartedState> mStartingUsers = new ArrayList<UserStartedState>();

    /**
     * Set when the system is going to sleep, until we have successfully paused
     * the current activity and released our wake lock. At that point the system
     * is allowed to actually sleep.
     */
    final PowerManager.WakeLock mGoingToSleep;

    /**
     * We don't want to allow the device to go to sleep while in the process of
     * launching an activity. This is primarily to allow alarm intent receivers
     * to launch an activity and get that to run before the device goes back to
     * sleep.
     */
    ...
    /**
     * This is the last activity that we put into the paused state. This is used
     * to determine if we need to do an activity transition while sleeping, when
    ...
    /**
     * Set when we know we are going to be calling updateConfiguration() soon,
     * so want to skip intermediate config checks.
     */
    boolean mConfigWillChange;

    /**
     * Set to indicate whether to issue an onUserLeaving callback when a newly
     * launched activity is being brought in front of us.
     */
    ...
    /**
     * Save the most recent screenshot for reuse. This keeps Recents from taking
     * two identical screenshots, one for the Recents thumbnail and one for the
     * pauseActivity thumbnail.
     */

    /**
     * Is the data protection currently enabled?
     */
    String mDataProtectionPackageName = null;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case PAUSE_TIMEOUT_MSG: {
                    ...
                    // We don't at this point know if the activity is
                    // fullscreen,
    ...
    /**
     * This is a simplified version of topRunningActivityLocked that provides a
     * number of optional skip-over modes. It is intended for use with the
     * ActivityController hook only.
     * 
     * @param token If non-null, any history records matching this token will be
     *            skipped.
     * @param taskId If non-zero, we'll attempt to skip over records with the
     *            same task ID.
    ...
    final ActivityRecord topRunningActivityLocked(IBinder token, int taskId) {
        ...
        while (i >= 0) {
            ActivityRecord r = mHistory.get(i);
            // Note: the taskId check depends on real taskId fields being
            // non-zero
    ...
    /**
     * Returns the first activity (starting from the top of the stack) that is
     * the same as the given activity. Returns null if no such activity is
     * found.
     */
    ...
    /*
     * Move the activities around in the stack to bring a user to the
     * foreground.
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        if (andResume) {
            ...
            if (DEBUG_STATES)
                Slog.v(TAG, "Moving to RESUMED: " + r
        ...
        if (!mSleepTimeout) {
            if (mResumedActivity != null) {
                ...
                if (DEBUG_USER_LEAVING)
                    Slog.v(TAG, "Sleep => pause with userLeaving=false");
            ...
            if (mStoppingActivities.size() > 0) {
                ...
                if (DEBUG_PAUSE)
                    Slog.v(TAG, "Sleep still need to stop "
            ...
            if (mGoingToSleepActivities.size() > 0) {
                ...
                if (DEBUG_PAUSE)
                    Slog.v(TAG, "Sleep still need to sleep "
    ...
    private final void startPausingLocked(boolean userLeaving, boolean uiSleeping) {
        ...
        if (!mService.mSleeping && !mService.mShuttingDown) {
            ...
            if (!mHandler.hasMessages(LAUNCH_TIMEOUT_MSG)) {
                // To be safe, don't allow the wake lock to be held for too
                // long.
    ...
    final void activityResumed(IBinder token) {
        ...
        synchronized (mService) {
            ...
            if (index >= 0) {
                ...
                if (DEBUG_SAVED_STATE)
                    Slog.i(TAG, "Resumed activity; dropping state of: " + r);
                r.icicle = null;
                r.haveState = false;
            }
        }
    ...
    final void activityStoppedLocked(ActivityRecord r, Bundle icicle, Bitmap thumbnail,
        ...
        if (DEBUG_SAVED_STATE)
            Slog.i(TAG, "Saving icicle of " + r + ": " + icicle);
        if (icicle != null) {
        ...
        if (!r.stopped) {
            if (DEBUG_STATES)
                Slog.v(TAG, "Moving to STOPPED: " + r + " (stop complete)");
            mHandler.removeMessages(STOP_TIMEOUT_MSG, r);
            r.stopped = true;
            r.state = ActivityState.STOPPED;
            if (r.finishing) {
                ...
                if (r.configDestroy) {
                    ...
                    // Now that this process has stopped, we may want to
                    // consider
    ...
    /**
     * Once we know that we have asked an application to put an activity in the
     * resumed state (either by launching it or explicitly telling it), this
     * function updates the rest of our state to match that fact.
     */
    private final void completeResumeLocked(ActivityRecord next) {
        ...
        //Update the data protection notification for the activity
        updateDataProtectionNotificationLocked(next);
    }
     ...
     * 
     * @param prev The previously resumed activity, for when in the process of
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        // than stopped+finished because the device went to sleep, we need to
        // make
        ...
        if (prev != null && prev != next) {
            if (!prev.waitingVisible && next != null && !next.nowVisible) {
                ...
                if (prev.finishing) {
                    ...
                    if (DEBUG_SWITCH)
                        Slog.v(TAG, "Previous already visible but still waiting to hide: "
                                + prev + ", waitingVisible="
                                + (prev != null ? prev.waitingVisible : null)
                                + ", nowVisible=" + next.nowVisible);
                }
    ...
    private final void updateDataProtectionNotificationLocked(ActivityRecord next) {

        if (mDataProtectionPackageName != null && mDataProtectionPackageName.equals(next.packageName)) {
            return;
        }

        // Get privacy settings for the package
        boolean privacy = mService.mAppOpsService.getDataProtectionSettingForPackage(
                next.app.uid, next.packageName);

        if (mDataProtectionPackageName != null && !privacy) {
            Message msg = mService.mHandler.obtainMessage(
                    ActivityManagerService.CANCEL_PRIVACY_NOTIFICATION_MSG, next.userId);
            msg.sendToTarget();
            mDataProtectionPackageName = null;
        } else if (privacy) {
            Message msg = mService.mHandler.obtainMessage(
                    ActivityManagerService.POST_PRIVACY_NOTIFICATION_MSG, next);
            msg.sendToTarget();
            mDataProtectionPackageName = next.packageName;
        }
    }
    ...
    private final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        if (NH > 0) {
            ...
            if (SHOW_APP_STARTING_PREVIEW && doShow) {
                // Figure out if we are transitioning from another activity that
                // is
    ...
    private final ActivityRecord resetTaskIfNeededLocked(ActivityRecord taskTop,
        ...
        for (int i = mHistory.size() - 1; i >= -1; i--) {
            ...
            // Don't check any lower in the stack if we're crossing a user
            // boundary.
            ...
            if (target.task == task) {
                ...
                if (below != null && below.task == task) {
                    ...
                    if (!finishOnTaskLaunch && !clearWhenTaskReset && target.resultTo != null) {
                        ...
                        if (target.taskAffinity != null
                            ...
                            if (DEBUG_TASKS)
                                Slog.v(TAG, "Start pushing activity " + target
                        ...
                        for (int srcPos = targetI; srcPos <= replyChainEnd; srcPos++) {
                            ...
                            if (DEBUG_TASKS)
                                Slog.v(TAG, "Pushing next activity " + p
                ...
                if (forceReset || finishOnTaskLaunch) {
                    ...
                    ActivityRecord p = null;
                    if (DEBUG_TASKS)
                        Slog.v(TAG, "Finishing task at index "
                    ...
                    if (DEBUG_TASKS)
                        Slog.v(TAG, "Reparenting task at index "
                                + targetI + " to " + replyChainEnd);
     ...
     * {@link Intent#FLAG_ACTIVITY_CLEAR_TOP}: search from the top of the stack
     * to the given task, then look for an instance of that activity in the
     * stack and, if found, finish all activities on top of it and return the
     * instance.
     * 
     * @param newR Description of the new activity being started.
     * @return Returns the old activity that should be continued to be used, or
    ...
    /**
     * Find the activity in the history stack within the given task. Returns the
     * index within the history at which it's found, or < 0 if not found.
     */
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (mMainStack) {
            ...
            if (mService.mDidAppSwitch) {
                ...
                // home (switches disabled, switch to home, mDidAppSwitch now
                // true);
                // user taps a home icon (coming from home so allowed, we hit
                // here
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
        ...
        synchronized (mService) {
            ...
            if (mMainStack && aInfo != null &&
                ...
                // This may be a heavy-weight process! Check to see if we
                // already
                ...
                if (aInfo.processName.equals(aInfo.applicationInfo.packageName)) {
                    if (mService.mHeavyWeightProcess != null &&
                        ...
                        IIntentSender target = mService.getIntentSenderLocked(
                                ...
                                realCallingUid, userId, null, null, 0, new Intent[] {
                                        intent
                                },
                                new String[] {
                                        resolvedType
    ...
    void sendActivityResultLocked(int callingUid, ActivityRecord r,
        ...
        if (DEBUG_RESULTS)
            Slog.v(TAG, "Send activity result to " + r
    ...
    private final void stopActivityLocked(ActivityRecord r) {
        ...
        if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_NO_HISTORY) != 0
            ...
            if (!r.finishing) {
                if (!mService.mSleeping) {
                    ...
                    if (DEBUG_STATES)
                        Slog.d(TAG, "Not finishing noHistory " + r
        ...
        if (r.app != null && r.app.thread != null) {
            ...
            try {
                r.stopped = false;
                if (DEBUG_STATES)
                    Slog.v(TAG, "Moving to STOPPING: " + r
                            + " (stop requested)");
                ...
                r.stopped = true;
                if (DEBUG_STATES)
                    Slog.v(TAG, "Stop failed; moving to STOPPED: " + r);
                r.state = ActivityState.STOPPED;
                if (r.configDestroy) {
    ...
    final ArrayList<ActivityRecord> processStoppingActivitiesLocked(
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (localLOGV)
                Slog.v(TAG, "Stopping " + s + ": nowVisible="
    ...
    final ActivityRecord activityIdleInternal(IBinder token, boolean fromTimeout,
        ...
        synchronized (mService) {
            ...
            if (index >= 0) {
                ...
                // If this activity is fullscreen, set up to hide those under
                // it.
                ...
                // Slog.i(TAG, "IDLE: mBooted=" + mBooted + ", fromTimeout=" +
                // fromTimeout);
                if (mMainStack) {
                    if (!mService.mBooted) {
    ...
    /**
     * @return Returns true if the activity is being finished, false if for some
     *         reason it is being left as-is.
     */
    ...
    final void finishActivityResultsLocked(ActivityRecord r, int resultCode, Intent resultData) {
        ...
        if (resultTo != null) {
            if (DEBUG_RESULTS)
                Slog.v(TAG, "Adding result to " + resultTo
            ...
            Slog.v(TAG, "No result destination from " + r);
     ...
     *         list, or false if it is still in the list and will be removed
     *         later.
     */
        ...
        if (index < (mHistory.size() - 1)) {
            ...
            if (next.task == r.task) {
                ...
                if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET) != 0) {
                    ...
                    // be cleared when the task is reset, don't lose that
                    // information,
    ...
    /**
     * Perform the common clean-up of an activity record. This is called both as
     * part of destroyActivityLocked() (when destroying the client-side
    ...
    final void cleanUpActivityLocked(ActivityRecord r, boolean cleanServices,
        ...
        if (setState) {
            if (DEBUG_STATES)
                Slog.v(TAG, "Moving to DESTROYED: " + r + " (cleaning up)");
            r.state = ActivityState.DESTROYED;
            if (DEBUG_APP)
                Slog.v(TAG, "Clearing app during cleanUp for activity " + r);
            r.app = null;
        }
    ...
    final void removeActivityFromHistoryLocked(ActivityRecord r) {
        ...
        removeTimeoutsForActivityLocked(r);
        if (DEBUG_STATES)
            Slog.v(TAG, "Moving to DESTROYED: " + r
                    + " (removed from history)");
        r.state = ActivityState.DESTROYED;
        if (DEBUG_APP)
            Slog.v(TAG, "Clearing app during remove for activity " + r);
        r.app = null;
    ...
    final void destroyActivitiesLocked(ProcessRecord owner, boolean oomAdj, String reason) {
        ...
        for (int i = mHistory.size() - 1; i >= 0; i--) {
            ...
            if (r.app != null && r != mResumedActivity && r != mPausingActivity
                    ...
                    && r.state != ActivityState.DESTROYED) {
                if (DEBUG_SWITCH)
                    Slog.v(TAG, "Destroying " + r + " in state " + r.state
     ...
     * called both when actually finishing an activity, or when performing a
     * configuration switch where we destroy the current client-side object but
     * then create a new client-side object for this same HistoryRecord.
     */
    final boolean destroyActivityLocked(ActivityRecord r,
        ...
        if (hadApp) {
            ...
            if (r.finishing && !skipDestroy) {
                if (DEBUG_STATES)
                    Slog.v(TAG, "Moving to DESTROYING: " + r
                            + " (destroy requested)");
                r.state = ActivityState.DESTROYING;
        ...
        while (i > 0) {
            ...
            if (r.app == app) {
                ...
                if ((!r.haveState && !r.stateNotNeeded) || r.finishing) {
                    ...
                    // We have launched this activity too many times since it
                    // was
                ...
                if (remove) {
                    ...
                    if (DEBUG_APP)
                        Slog.v(TAG, "Clearing app during removeHistory for activity "
                                + r);
                    r.app = null;
                    r.nowVisible = false;
                    if (!r.haveState) {
                        if (ActivityStack.DEBUG_SAVED_STATE)
                            Slog.i(TAG,
    ...
    /**
     * Move the current home activity's task (if one exists) to the front of the
     * stack.
     */
    ...
    final void moveTaskToFrontLocked(TaskRecord tr, ActivityRecord reason, Bundle options) {
        ...
        while (pos >= 0) {
            ...
            if (r.task.taskId == task) {
                if (localLOGV)
                    Slog.v(TAG, "Removing and adding at " + top);
                if (DEBUG_ADD_REMOVE) {
                    RuntimeException here = new RuntimeException("here");
                    here.fillInStackTrace();
    ...
    /**
     * Worker method for rearranging history stack. Implements the function of
     * moving all activities for a specific task (gathering them if disjoint)
     * into a single group at the bottom of the stack. If a watcher is
     * installed, the action is preflighted and the watcher has an opportunity
    ...
    final boolean moveTaskToBackLocked(int task, ActivityRecord reason) {
        ...
        // First check
        // for *other* available tasks, but if none are available, then try
        // again allowing the
        ...
        while (pos < N) {
            ...
            if (r.task.taskId == task) {
                if (localLOGV)
                    Slog.v(TAG, "Removing and adding at " + (N - 1));
                if (DEBUG_ADD_REMOVE) {
                    RuntimeException here = new RuntimeException("here");
                    here.fillInStackTrace();
     ...
     * configuration is the same, or the activity will remain running as-is for
     * whatever reason. Ensures the HistoryRecord is updated with the correct
     * configuration and all other bookkeeping is handled.
     */
    final boolean ensureActivityConfigurationLocked(ActivityRecord r,
        ...
        if (r.app != null && r.app.thread != null) {
            try {
                if (DEBUG_CONFIGURATION)
                    Slog.v(TAG, "Sending new config to " + r);
